 ...
 *
 */
inline bool is_sync_point_full(PVM pVM, PVMCPU pVCpu)
{
    if (pVCpu == NULL)
        pVCpu = pVM->aCpus;
 ...
 */
inline struct sync_point *save_sync_point_int(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, bool is_irq, uint8_t exit_reason,
 ...
 * This returns NULL if we're not currently recording anything.
 */
inline struct sync_point *save_sync_point(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, uint8_t exit_reason)
{
 ...
 */
inline struct sync_point *save_irq(PVM pVM, PVMCPU pVCpu, PCPUMCTX pCtx, uint8_t ivt, uint32_t fault_error_code)
{
    return save_sync_point_int(pVM, pVCpu, pCtx, true, ivt, fault_error_code);
}
 ...
 */
inline struct saved_hardware_access *find_hardware_access(PVM pVM, PVMCPU pVCpu, uint32_t length)
{
 ...
 */
inline VBOXSTRICTRC save_hardware_access(PVM pVM, PVMCPU pVCpu,
    PPDMDEVINS pDevIns,
 ...
 */
inline VBOXSTRICTRC save_mmio_access(PVM pVM, PVMCPU pVCpu,
    PPDMDEVINS pDevIns,
 ...
 */
inline VBOXSTRICTRC save_pci_access(PVM pVM, PVMCPU pVCpu,
    PPDMDEVINS pDevIns,
    uint64_t address, unsigned int length, const uint8_t *data, bool write)
{
 ...
 */
inline VBOXSTRICTRC save_port_access(PVM pVM, PVMCPU pVCpu,
