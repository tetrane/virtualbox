...
static const char *reason(uint8_t code)
{
    ...
    case VMX_EXIT_XDTR_ACCESS: return "Access to GDTR or IDTR. Guest software attempted to execute LGDT, LIDT, SGDT, or SIDT.";
    case VMX_EXIT_TR_ACCESS: return "Access to LDTR or TR. Guest software attempted to execute LLDT, LTR, SLDT, or STR.";
...
void vm_tweaks::flush_data()
{
    ...
    while ((cur_read = ASMAtomicReadU32(&cpu->reven.sync_point_read_index)) != last_write
        ...
        // alert plugins
        plugin_manager_.sync_point_callback(&sync_point);
