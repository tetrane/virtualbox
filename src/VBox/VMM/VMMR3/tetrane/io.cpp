...
/**
 * Callback function for printing an MMIO range.
 *
 * @returns 0 (continue enum)
 * @param   pNode       Pointer to a IOMMMIORANGE node.
 * @param   pvUser      Pointer to the dumped file.
 */
static DECLCALLBACK(int) register_mmio(PAVLROGCPHYSNODECORE pNode, void *pvUser)
{
    return static_cast<vm_tweaks*>(pvUser)->register_mmio((PIOMMMIORANGE)pNode);
}

int vm_tweaks::register_mmio(PIOMMMIORANGE pRange)
{
    device &dev = get_device(pRange->pDevInsR3);

    memory_range range;

    if (pRange->pszDesc)
        range.description = pRange->pszDesc;
    range.physical_address = pRange->GCPhys;
    range.length = pRange->cb;
    range.instance = pRange->pDevInsR3->iInstance;
...
void vm_tweaks::register_port(PIOMIOPORTRANGER3 pRange)
{
...
void vm_tweaks::enable_io_dump()
{
    ...
    {
        PIOMIOPORTRANGER3 pRange = iomIOPortGetRange(vm, port);

        if (pRange)
        {
            register_port(pRange);

            port += pRange->cPorts - 1;
        }
    }
    IOM_UNLOCK_SHARED(vm);

    RTAvlroGCPhysDoWithAll(&vm->iom.s.pTreesR3->MMIOTree, true, ::register_mmio, this);
...
void vm_tweaks::read_port(RTIOPORT Port, uint32_t read_value, size_t cbValue, PIOMIOPORTRANGER3 pRange)
{
...
void vm_tweaks::write_port(RTIOPORT Port, uint32_t written_value, size_t cbValue, PIOMIOPORTRANGER3 pRange)
{
}
